...
def create_venue_submission():
  form = VenueForm()
  '''check to see if seeking artist is checked'''
  checked = ''
...
'''Delete a venue if it doesn't have any shows posted'''    
...
def delete_venue(venue_id):
  try:
    ...
    flash('This venue has an upcoming or past show(s), so it cannot be deleted')
  finally:
    db.session.close()
